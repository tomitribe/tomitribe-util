...
import static org.tomitribe.util.hash.SizeOf.instanceSize;
...
public final class BasicSliceInput
    ...
    public void setPosition(long position) {
        ...
        final int max = slice.length() + 1;
        if (position < 0 || position >= max) {
            throw new IndexOutOfBoundsException("position must be between 0 and " + max);
        }
